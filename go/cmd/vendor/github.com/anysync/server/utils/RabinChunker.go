...
const (
	...
	averageBits13 = 13
	splitmask13 = (1 << averageBits13) - 1

	averageBits16 = 16
	splitmask16 = (1 << averageBits16) - 1

	// aim to create chunks of 18 bits or about 256kb on average.
	averageBits18 = 18
	splitmask18 = (1 << averageBits18) - 1

	// aim to create chunks of 19 bits or about 512kb on average.
	averageBits19 = 19
	splitmask19 = (1 << averageBits19) - 1

	// aim to create chunks of 20 bits or about 1MiB on average.
	averageBits20 = 20
	splitmask20 = (1 << averageBits20) - 1

	// aim to create chunks of 22 bits or about 4MB on average.
	averageBits22 = 22
	splitmask22 = (1 << averageBits22) - 1

	// aim to create chunks of 24 bits or about 16MB on average.
	averageBits24 = 24
	splitmask24 = (1 << averageBits24) - 1

	chunkerBufSize = 128 * kiB
)
...
func (c *Chunker) Next(repository string, outputFile * bufio.Writer) (Chunk, error) {
	//data = data[:0]
	val := GetRepositoryNameHash(repository)
	hash := NewHashWithHeader(val);
	...
	minSize := c.MinSize
	maxSize := c.MaxSize
	var splitMask uint64;
	if(c.splitMask != 0) {
		splitMask = c.splitMask;
	}else{
		splitMask  = splitmask20;
	}

	Debugf("In Next, min:%d, max:%d, mask:%d\n", minSize, maxSize, splitMask)
	buf := c.buf
	for {
		if c.bpos >= c.bmax {
		...
		if c.pre > 0 {
			...
			if c.pre > uint(n) {
				c.pre -= uint(n)
				tmpBuf := buf[c.bpos:c.bmax]
				outputFile.Write(tmpBuf)
				hash.Write(tmpBuf)
			...
			tmpBuf:= buf[c.bpos:c.bpos+c.pre];
			outputFile.Write(tmpBuf)
			hash.Write(tmpBuf);
		...
		for _, b := range buf[c.bpos:c.bmax] {
			...
			if (digest&splitMask) == 0 || add >= maxSize {
				i := add - c.count - 1
				tmpBuf := c.buf[c.bpos:c.bpos+uint(i)+1]
				outputFile.Write(tmpBuf)
				hash.Write(tmpBuf)
				...
				c.reset()
				if(add >= maxSize){
					Debugf("!~~~~~~~~~~~~~~~~~~~~~~~~~~~~   Exceed max size:%d, no match. chunk.start:%d, count:%d\n", maxSize, chunk.Start, chunk.Length)
				}else {
					Debugf("Normal return 3 max size:%d, no match. chunk.start:%d, count:%d\n", maxSize, chunk.Start, chunk.Length)
				}
				return chunk, nil
			}
		}
		...
		steps := c.bmax - c.bpos
		if steps > 0 {
			tmpBuf := c.buf[c.bpos:c.bpos+steps];
			outputFile.Write(tmpBuf)
			hash.Write(tmpBuf)
...
func (c Chunk) toBytes() []byte {
	l := 20 + HASH_BYTE_COUNT //20 + 28
	buf := make([]byte, l)          //20 + 28
	PutUint64(buf, 0, uint64(c.Start))
	PutUint32(buf, 8, uint32(c.Length))
	PutUint64(buf, 12, uint64(c.Cut))
	for i := 20; i < l; i++ {
		buf[i] = c.Hash[i-20]
	}
	return buf
