...
func FileGetFlag(section, key string) (string, bool) {
	//zyb
	//log.Println("Enter configfileget, section:", section, "; key: ", key)
	c := utils.LoadAppParams().GetSelectedStorage();
	if key == "token" {
		rs := utils.GetRemotes()
		for _, val := range rs.Remote {

			if val.Name == section {
				return val.Data["token"], true
			}
		}
		return "",false
		...
		for _, val := range rs.Remote {
			if val.Name == section {
				return val.Data["account"], true
			}
		}
		return "",false
	} else if key == "secret_access_key" || key == "key" {
		k := c.Secret
		if k == ""{
			k = "dummy"
		}
		...
		rs := utils.GetRemotes()
		if len(rs.Remote) == 0 {
			log.Println("Error, no remotes found")
		}
		for _, val := range rs.Remote {
			if val.Name == section {
				if val.Type == "GoogleDrive" {
					return "drive", true;
				} else if val.Type == "Dropbox" {
					return "dropbox", true;
				} else if val.Type == utils.REMOTE_TYPE_SERVER ||  val.Type == utils.REMOTE_TYPE_S3 {
					return "s3", true;
				} else if  val.Type == utils.REMOTE_TYPE_OFFICIAL{
					return "s3", true;
				} else if val.Type == utils.REMOTE_TYPE_B2 {
					return "b2", true;
				}
